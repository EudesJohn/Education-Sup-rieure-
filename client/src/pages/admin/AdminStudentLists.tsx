/** Admin — Gestion des listes d'étudiants.
 *
 * L'admin peut créer, modifier et supprimer des listes d'étudiants
 * pour le compte des enseignants. Les professeurs n'ont accès qu'en
 * lecture seule aux listes qui leur sont attribuées.
 */
...
export function AdminStudentListsPage() {
  ...
  const fetchLists = async () => {
    ...
    setError('')
    try {
      const res = await adminApi.listStudentLists()
      setLists(res.data)
    } catch (err: any) {
...
function ListsTable({
  ...
  useEffect(() => {
    // Charger les noms des enseignants pour l'affichage
    adminApi.listTeachers().then(res => {
      const teachers: TeacherInfo[] = Array.isArray(res.data) ? res.data : []
  ...
  const handleDelete = async (e: React.MouseEvent, id: number) => {
    e.stopPropagation()
    if (!window.confirm('Supprimer cette liste ? Cette action est irreversible.')) return
    setDeletingId(id)
    try {
      await adminApi.deleteStudentList(id)
      onDeleted()
    } catch {
      setDeletingId(null)
    }
  ...
  if (lists.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <h3 className="text-lg font-medium text-white/70 mb-2">Aucune liste d'etudiants</h3>
        <p className="text-sm text-muted/50 max-w-md mx-auto mb-6">
          Creez votre premiere liste et attribuez-la a un enseignant.
        </p>
      </div>
    )
  }
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted/60 mt-1">
              {lst.student_count} etudiant{lst.student_count > 1 ? 's' : ''}
              {' · '}
              Enseignant : {teacherNames[lst.teacher_id] || `#${lst.teacher_id}`}
              {' · '}
...
function CreateListWizard({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  ...
  // Etudiants importes de la classe + ajouts manuels
  const [entries, setEntries] = useState<ManualEntry[]>([])
  // Ajout manuel temporaire
  const [manualName, setManualName] = useState('')
  const [manualNumber, setManualNumber] = useState('')

  // Charger les listes de reference au montage
  useEffect(() => {
    adminApi.listTeachers().then(r => setTeachers(Array.isArray(r.data) ? r.data : [])).catch(() => {})
    api.get('/teacher/institutions').then(r => setInstitutions(Array.isArray(r.data) ? r.data : [])).catch(() => {})
    api.get('/teacher/academic-years').then(r => setAcademicYears(Array.isArray(r.data) ? r.data : [])).catch(() => {})
    api.get('/teacher/study-levels').then(r => setStudyLevels(Array.isArray(r.data) ? r.data : [])).catch(() => {})
  }, [])

  // Gestionnaires hierarchiques
  const handleInstitutionChange = async (instId: number | '') => {
    ...
    if (instId) {
      try {
        const res = await api.get('/teacher/filieres', { params: { institution_id: instId as number } })
  ...
  const handleFiliereChange = async (filiereId: number | '') => {
    ...
    if (filiereId && selectedYear) {
      try {
        const res = await api.get('/teacher/classes', { params: { filiere_id: filiereId as number, academic_year_id: selectedYear as number } })
  ...
  const handleYearChange = async (yearId: number | '') => {
    ...
    if (yearId && selectedFiliere) {
      try {
        const res = await api.get('/teacher/classes', { params: { filiere_id: selectedFiliere as number, academic_year_id: yearId as number } })
        setClasses(Array.isArray(res.data) ? res.data : [])
      } catch { /* ignore */ }
    }
  }
    ...
    if (selectedFiliere && selectedYear && studyLevelId) {
      try {
        const res = await api.get('/teacher/classes', {
          params: { filiere_id: selectedFiliere as number, academic_year_id: selectedYear as number, study_level_id: studyLevelId as number },
        })
        setClasses(Array.isArray(res.data) ? res.data : [])
      } catch { /* ignore */ }
    }
  ...
  const handleClassSelect = async (classId: number | '') => {
    ...
    if (classId) {
      ...
      try {
        const res = await api.get(`/admin/classes/${classId as number}/students`)
        const students: ClassStudent[] = Array.isArray(res.data) ? res.data : []
        setClassStudents(students)
        // Auto-populer les entrees avec les eleves de la classe
  ...
  // Ajouter un etudiant manquant
  const addManualStudent = () => {
    const name = manualName.trim()
    const num = manualNumber.trim()
    if (!name || !num) return
    if (entries.some(e => e.student_number === num)) {
      setError(`L'etudiant avec le matricule "${num}" est deja dans la liste.`)
      return
    }
    setEntries(prev => [...prev, { student_name: name, student_number: num, email: '', class_name: '' }])
    setManualName('')
    setManualNumber('')
    setError('')
  }
  ...
  const handleSave = async () => {
    if (!listName.trim()) {
      setError('Donne un nom a la liste.')
      return
    }
    if (!selectedTeacherId) {
      setError('Selectionne un enseignant pour cette liste.')
      return
    }
    const valid = entries.filter(e => e.student_name && e.student_number)
    if (valid.length === 0) {
      setError('Selectionne une classe ou ajoute au moins un etudiant.')
      return
    }
    setSaving(true)
    setError('')
    try {
      // 1. Creer la liste avec les entrees en une seule requete
      const res = await adminApi.confirmStudentList({
        name: listName.trim(),
  ...
  if (step === 'done') {
    return (
      <div className="card p-10 text-center animate-scale-in">
        ...
        <h3 className="text-xl font-semibold text-white mb-2">Liste creee avec succes !</h3>
        <p className="text-sm text-muted/60 mb-6">
          {entries.length} etudiant{entries.length > 1 ? 's' : ''} enregistre{entries.length > 1 ? 's' : ''}.
        </p>
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      {/* En-tete : infos de la liste */}
      <div className="card p-5">
        <h3 className="text-lg font-semibold text-white mb-4">Nouvelle liste d'etudiants</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            ...
            <input type="text" value={listName} onChange={e => setListName(e.target.value)}
              className="input w-full" placeholder="ex: L2 Informatique 2025-26" autoFocus />
          </div>
          <div>
            ...
            <input type="text" value={groupe} onChange={e => setGroupe(e.target.value)}
              className="input w-full" placeholder="ex: Groupe A, TD1..." />
          </div>
          <div>
            <label className="block text-xs font-medium text-muted/70 mb-1.5">
              Enseignant <span className="text-rose-accent">*</span>
            </label>
            <select value={selectedTeacherId} onChange={e => setSelectedTeacherId(e.target.value ? Number(e.target.value) : '')} className="input text-sm">
      ...
      {/* Selection de la classe — toujours visible */}
      <div className="card p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <div>
            <label className="block text-xs font-medium text-muted/70 mb-1">Etablissement</label>
          ...
          <div>
            <label className="block text-xs font-medium text-muted/70 mb-1">Annee academique</label>
            <select value={selectedYear} onChange={e => handleYearChange(e.target.value ? Number(e.target.value) : '')} className="input text-sm">
              <option value="">— Selectionner —</option>
              {academicYears.map(y => <option key={y.id} value={y.id}>{y.name}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-muted/70 mb-1">Filiere</label>
          ...
          <div>
            <label className="block text-xs font-medium text-muted/70 mb-1">Niveau d'etude</label>
          ...
          <div>
            <label className="block text-xs font-medium text-muted/70 mb-1">Specialite</label>
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-3">
          <h4 className="font-semibold text-white">
            Etudiants
            {entries.length > 0 && (
              <span className="ml-2 text-sm font-normal text-muted/60">({entries.length})</span>
            )}
          </h4>
          ...
          <div className="flex items-center gap-2">
            <input type="text" value={manualName} onChange={e => setManualName(e.target.value)}
              className="input text-sm py-1.5 w-40" placeholder="Nom complet"
              onKeyDown={e => e.key === 'Enter' && addManualStudent()} />
            <input type="text" value={manualNumber} onChange={e => setManualNumber(e.target.value)}
              className="input text-sm py-1.5 w-28" placeholder="Matricule"
              onKeyDown={e => e.key === 'Enter' && addManualStudent()} />
            <button onClick={addManualStudent} disabled={!manualName.trim() || !manualNumber.trim()}
              className="btn-primary text-sm px-3 py-1.5 disabled:opacity-50 flex items-center gap-1">
        ...
        {loadingClassStudents && (
          <div className="text-center py-8 text-sm text-muted/60">Chargement des etudiants...</div>
        )}

        {!loadingClassStudents && entries.length === 0 && (
          <div className="text-center py-8">
            <p className="text-sm text-muted/50">
              {selectedClass
                ? 'Aucun etudiant trouve dans cette classe.'
                : 'Selectionne une classe ci-dessus pour voir les etudiants.'}
            </p>
          </div>
        )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.06] bg-white/[0.02]">
                  <th className="text-left py-2 px-3 text-xs font-medium text-muted/60 uppercase tracking-wider">#</th>
              ...
              <tbody>
                {entries.map((entry, idx) => (
                  <tr key={idx} className="border-b border-white/[0.03] last:border-0 group">
                    ...
                    <td className="py-2 px-3 text-white/90">{entry.student_name}</td>
                    <td className="py-2 px-3 text-muted/70 font-mono text-xs">{entry.student_number}</td>
                    <td className="py-2 px-3 text-muted/50 text-xs">{entry.email || '—'}</td>
                    <td className="py-2 px-2 w-8">
                      <button onClick={() => removeEntry(idx)}
                        className="p-1 rounded-lg text-muted/30 hover:text-rose-accent hover:bg-rose-accent/10 transition-all opacity-0 group-hover:opacity-100"
                        title="Retirer">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm text-muted/60">
          {entries.length} etudiant{entries.length > 1 ? 's' : ''}
        </span>
        <div className="flex items-center gap-3">
          <button onClick={onCancel} className="btn-ghost text-sm px-5 py-2.5">Annuler</button>
          <button onClick={handleSave} disabled={saving || !listName.trim() || entries.length === 0 || !selectedTeacherId}
            className="btn-primary text-sm px-6 py-2.5 disabled:opacity-50">
            {saving ? 'Enregistrement...' : 'Creer la liste'}
          </button>
        </div>
      </div>
    </div>
...
function DetailView({
  ...
  const fetchDetail = async () => {
    ...
    try {
      const [lstRes, entriesRes] = await Promise.all([
        adminApi.getStudentList(listId),
        adminApi.getListEntries(listId),
      ])
  ...
  // Gestionnaires import depuis une classe (vue detail)
  const handleInstChange_detail = async (id: number | '') => {
    ...
    if (id) {
      try { const r = await api.get('/teacher/filieres', { params: { institution_id: id as number } }); setFils(Array.isArray(r.data) ? r.data : []) } catch {}
    }
  }
  const handleFilChange_detail = async (id: number | '') => {
    setSelFil(id); setSelStudyLevel(''); setSelCls(''); setClsList([]); setClsStudents([])
    if (id && selYear) {
      try { const r = await api.get('/teacher/classes', { params: { filiere_id: id as number, academic_year_id: selYear as number } }); setClsList(Array.isArray(r.data) ? r.data : []) } catch {}
    }
  }
  const handleYearChange_detail = async (id: number | '') => {
    setSelYear(id); setSelStudyLevel(''); setSelCls(''); setClsList([]); setClsStudents([])
    if (id && selFil) {
      try { const r = await api.get('/teacher/classes', { params: { filiere_id: selFil as number, academic_year_id: id as number } }); setClsList(Array.isArray(r.data) ? r.data : []) } catch {}
    }
  }
  const handleStudyLevelChange_detail = async (id: number | '') => {
    ...
    if (selFil && selYear && id) {
      try { const r = await api.get('/teacher/classes', { params: { filiere_id: selFil as number, academic_year_id: selYear as number, study_level_id: id as number } }); setClsList(Array.isArray(r.data) ? r.data : []) } catch {}
    }
  }
  ...
  const handleClsSelect_detail = async (id: number | '') => {
    ...
    if (id) {
      ...
      try { const r = await api.get(`/admin/classes/${id as number}/students`); setClsStudents(Array.isArray(r.data) ? r.data : []) } catch {}
  ...
  const importFromClass_detail = async () => {
    for (const s of clsStudents) {
      try {
        await adminApi.addListEntry(listId, {
          student_name: s.student_name,
          student_number: s.student_number,
  ...
  const handleDeleteEntry = async (entryId: number) => {
    if (!window.confirm('Supprimer cet etudiant de la liste ?')) return
    try {
      await adminApi.deleteListEntry(listId, entryId)
  ...
  const saveEdit = async (entryId: number) => {
    ...
    setError('')
    try {
      await adminApi.updateListEntry(listId, entryId, payload)
      setEditingId(null)
      setEditForm({})
      fetchDetail()
    } catch (err: any) {
  ...
  return (
    <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-2.5 mt-1.5">
              ...
              <span className="text-xs text-muted/60">
                {list.student_count} etudiant{list.student_count > 1 ? 's' : ''}
              </span>
              <span className="text-xs text-muted/40">
                · Enseignant : {teacherNames[list.teacher_id] || `#${list.teacher_id}`}
              </span>
              <span className="text-xs text-muted/40">
                · Creee le {new Date(list.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}
              </span>
            </div>
          </div>
          <button
      ...
      {/* Import depuis une classe (vue detail) */}
        ...
        <button
          ...
          onClick={() => {
            ...
            if (!showImportClass && insts.length === 0) {
              api.get('/teacher/institutions').then(r => setInsts(Array.isArray(r.data) ? r.data : [])).catch(() => {})
              api.get('/teacher/academic-years').then(r => setYears(Array.isArray(r.data) ? r.data : [])).catch(() => {})
              api.get('/teacher/study-levels').then(r => setDetailStudyLevels(Array.isArray(r.data) ? r.data : [])).catch(() => {})
            }
          }}
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {clsStudents.length > 0 && (
              <span className="badge-amber text-[10px] px-2 py-0.5">{clsStudents.length} etudiant(s)</span>
            )}
          </div>
        ...
        {showImportClass && (
          <div className="mt-4 space-y-4 animate-fade-in">
            ...
            {!loadClsStudents && selCls && clsStudents.length === 0 && (
              <div className="text-center py-3 text-sm text-muted/50">Aucun etudiant dans cette classe.</div>
            )}
              ...
              <>
                ...
                <div className="flex justify-end">
                  <button onClick={importFromClass_detail} className="btn-primary text-sm px-4 py-2 flex items-center gap-1.5">
                    ...
                    Ajouter {clsStudents.length} etudiant(s)
                  </button>
                </div>
              </>
            )}
      ...
      {showAddForm && (
        <div className="card p-5 animate-fade-in border border-neon-cyan/20">
          <h4 className="font-medium text-white mb-4">Nouvel etudiant</h4>
      ...
      <div className="card p-5">
        ...
        {entries.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-muted/50">Aucun etudiant dans cette liste</p>
            <p className="text-xs text-muted/40 mt-1">Cliquez sur « Ajouter un etudiant » pour commencer.</p>
          </div>
        ) : (
